...
# Import Algo Trading Engine
try:
    from algo_engine import AlgoEngine, create_algo_tab_content
...
try:
    ...
    # Initialize Algo Engine in session state
    if "algo_engine" not in st.session_state:
        st.session_state.algo_engine = AlgoEngine(
            kite_manager=st.session_state.get("kite_manager"),
            data_manager=data_manager,
            trader=trader
        ) if ALGO_ENGINE_AVAILABLE else None
...
# ======= RantvAlgo Paper Trading Integration =======
PAPER_TRADING_ONLY = True
from dataclasses import dataclass, field as _field_rantv
from datetime import datetime as _dt_rantv
from typing import List as _List_rantv
import csv as _csv_rantv, os as _os_rantv, random as _random_rantv, smtplib as _smtplib_rantv
from email.message import EmailMessage as _EmailMessage_rantv
from reportlab.platypus import SimpleDocTemplate as _SimpleDocTemplate_rantv, Paragraph as _Paragraph_rantv, Table as _Table_rantv
from reportlab.lib.styles import getSampleStyleSheet as _getSampleStyleSheet_rantv
from reportlab.lib.pagesizes import A4 as _A4_rantv
...
@dataclass
class PaperLedger:
    trades: _List_rantv[PaperTrade] = _field_rantv(default_factory=list)

    def add(self, trade: PaperTrade):
    ...
    def expectancy(self):
        ...
        win_prob = len(wins) / len(self.trades) if self.trades else 0
        expectancy = (win_prob * avg_win) - ((1 - win_prob) * avg_loss)
        return round(expectancy, 2)

class ShadowComparator:
    @staticmethod
    def simulate_live_exit(entry, action):
        slip = _random_rantv.uniform(0.0005, 0.002)
        if action == "BUY":
            return round(entry * (1 - slip), 2)
        else:
            return round(entry * (1 + slip), 2)
...
class PaperTrader:
    ...
    def execute_trade(self, symbol, action, quantity, price, stop_loss, target, win_probability, auto_trade, strategy):
        paper_exit = round(price * (1 + _random_rantv.uniform(-0.004, 0.006)), 2)
        live_exit = ShadowComparator.simulate_live_exit(price, action)
        if action == "BUY":
            paper_pnl = (paper_exit - price) * quantity
            live_pnl = (live_exit - price) * quantity
        else:
            paper_pnl = (price - paper_exit) * quantity
            live_pnl = (price - live_exit) * quantity
        trade = PaperTrade(
            time=_dt_rantv.now(),
            symbol=symbol,
            action=action,
            qty=quantity,
            entry_price=price,
            exit_price=paper_exit,
            pnl=round(paper_pnl, 2),
            strategy=strategy,
        )
        trade.live_pnl = round(live_pnl, 2)
...
class DailyReportGenerator:
    def __init__(self, ledger: PaperLedger):
        self.ledger = ledger
        self.date = _dt_rantv.now().strftime("%Y-%m-%d")
        self.base = f"paper_report_{self.date}"

    def export_csv(self):
        path = f"{self.base}.csv"
        with open(path, "w", newline="") as f:
            writer = _csv_rantv.writer(f)
            writer.writerow([
                "Time","Symbol","Action","Qty",
                "Entry","Exit(Paper)","PnL(Paper)","PnL(Live Shadow)","Strategy"
            ])
            for t in self.ledger.trades:
                writer.writerow([
                    ...
                    t.exit_price,
                    t.pnl,
                    getattr(t, "live_pnl", 0),
                    t.strategy,
                ])
        return path

    def export_pdf(self):
        path = f"{self.base}.pdf"
        doc = _SimpleDocTemplate_rantv(path, pagesize=_A4_rantv)
        styles = _getSampleStyleSheet_rantv()
        elements = []
        elements.append(_Paragraph_rantv(f"<b>Rantv Algo  Paper Trading Report ({self.date})</b>", styles["Title"]))
        elements.append(_Paragraph_rantv(f"Total P&L: {self.ledger.total_pnl()}", styles["Normal"]))
        elements.append(_Paragraph_rantv(f"Win Rate: {self.ledger.win_rate()}%", styles["Normal"]))
        elements.append(_Paragraph_rantv(f"Expectancy: {self.ledger.expectancy()}", styles["Normal"]))
        table_data = [["Time","Symbol","Action","Qty","Entry","Exit","Paper PnL","Live Shadow PnL"]]
        for t in self.ledger.trades:
            table_data.append([
                t.time.strftime("%H:%M"),
                t.symbol,
                ...
                f"{t.exit_price:.2f}",
                f"{t.pnl:.2f}",
                f"{getattr(t, 'live_pnl', 0):.2f}",
            ])
        elements.append(_Table_rantv(table_data))
        doc.build(elements)
        return path
...
class EmailSender:
    ...
    def send(subject, body, attachments):
        try:
            msg = _EmailMessage_rantv()
            msg["From"] = _os_rantv.getenv("REPORT_EMAIL_FROM")
            msg["To"] = _os_rantv.getenv("REPORT_EMAIL_TO")
            msg["Subject"] = subject
            ...
            for file in attachments:
                with open(file, "rb") as f:
                    msg.add_attachment(
                        ...
                        filename=_os_rantv.path.basename(file),
                    )
            server = _smtplib_rantv.SMTP(_os_rantv.getenv("SMTP_SERVER"), int(_os_rantv.getenv("SMTP_PORT", 587)))
            server.starttls()
            server.login(_os_rantv.getenv("SMTP_USER"), _os_rantv.getenv("SMTP_PASSWORD"))
