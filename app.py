...
if AUTO_TRADING_ENABLED:
    success, reason = execute_autonomous_trade(
        signal=signal,
        kite_mgr=kite_manager,
        risk_mgr=risk_manager,
        positions=open_positions
    )

    if success:
        st.success(f"🤖 AUTO TRADE EXECUTED: {signal['symbol']}")
    else:
        if st.session_state.get("debug", False):
            st.warning(f"AUTO TRADE SKIPPED: {reason}")

today = datetime.now().date()
if st.session_state.get("auto_trade_date") != today:
    auto_trade_state["trades_today"] = 0
    st.session_state["auto_trade_date"] = today

st.sidebar.markdown("## 🤖 Autonomous Trading")

AUTO_TRADING_ENABLED = st.sidebar.toggle(
    "Enable Auto Trading",
    value=False
)

if AUTO_TRADING_ENABLED and auto_trade_state["enabled_at"] is None:
    auto_trade_state["enabled_at"] = datetime.now()
